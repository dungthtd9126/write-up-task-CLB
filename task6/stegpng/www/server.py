    ...
    if request.method == 'POST':
        if 'file' in request.files and 'passphrase' in request.form:
            ...
            try:
                ...
                # Added strace to dump all syscalls to /tmp/trace.txt
                subprocess.run(['strace', '-f', '-o', '/tmp/trace.txt', 'steghide', 'extract', '-sf', stegfile_name, '-p', request.form['passphrase'], '-xf', outfile_name], check=True, timeout=60)
            except Exception:
                os.chdir('..')
